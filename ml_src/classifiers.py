...
try:
    from ml_src.preprocessing import make_dsets,makeDsets, get_label_idx_to_name, image_loader, default_loader, get_transforms
except ImportError:
    from preprocessing import make_dsets, makeDsets, get_label_idx_to_name, image_loader, default_loader, get_transforms
...
def get_pretrained_model(arch="resnet18", pop_last_pool_layer=False, use_gpu=False):
    ...
    if use_gpu:
        ...
        pretrained_features = torch.nn.DataParallel(pretrained_features.cuda())
...
class AttributeFCN(nn.Module):
    ...
    def forward(self, x):
        ...
        classes_softmax = F.log_softmax(average_pool_flatten, dim=1)
        ...
        for phase in phases:
            ...
            for data in dset_loader:
                ...
                if use_gpu:
                    ...
                    # inputs, labels = torch.nn.DataParallel(inputs).cuda(), torch.nn.DataParallel(labels).cuda()
                if phase == "train":
    ...
    for attrib_name, model in attribute_models.items():
        ...
        for batch_idx, dset in enumerate(dset_loader):
            ...
            if attribute_idx_map:
                pred_class = attribute_idx_map[attrib_name].get(str(pred_idx))
                if pred_class:
                    results[attrib_name] = {
                        "pred_idx": pred_idx,
...
def getImageResult(model, pretrained_model, image_url, attrib_name,
                attribute_idx_map=None, use_gpu=None):
...
def evaluateModel(model, pretrained_model, images_folder,batch_size=32, num_workers=4, use_gpu=None,flatten_pretrained_out=False):

    if use_gpu is None:
        use_gpu = torch.cuda.is_available()
        
    dset_loader = makeDsets(images_folder, is_train=False, shuffle=False, batch_size=batch_size, num_workers=num_workers)
...
def predict_attributes(image_url, pretrained_model, attribute_models, attribute_idx_map=None, 
    ...
    if use_gpu is None:
        use_gpu = torch.cuda.is_available()
    print("Loading Image path : ", image_url)
    image_features = image_loader(image_url, transforms=None, use_gpu=use_gpu)
    ...
    for attrib_name, model in attribute_models.items():
        ...
        pred_prob = float(np.exp(pred_prob.numpy().flatten()[0]))
...
def create_attributes_model(ModelClass, in_dims, pretrained_features, target_columns, weights_root,
    ...
    for col_name, col_dim in target_columns.items():
        print("Processing Attribute: {}".format(col_name))
        weights_path = os.path.join(weights_root, col_name + time.strftime("%Y%m%d-%H%M%S") + ".pth")
        load_weights_path = None
        if os.path.exists(weights_path):
            load_weights_path = weights_path
...
def create_all_attribute_models(ModelClass, in_dims, pretrained_features, target_columns, weights_root, batch_size=32, num_workers=4, num_epochs=10, use_gpu=None):
    models = {}
    for col_name, col_dim in target_columns.items():
        #print("Processing Attribute: {}".format(col_name))

        weights_folder_path = weights_root[col_name]
        weights_path = os.path.join(weights_folder_path, col_name+".pth")
        load_weights_path = None
        if os.path.exists(weights_path):
            load_weights_path = weights_path
        model = load_model(ModelClass, in_dims, col_dim, weights_path=load_weights_path, use_gpu=use_gpu)
...
def create_attributes_model_for_test(ModelClass, in_dims, target_columns, weights_root,use_gpu=None):
    models = {}
    print(in_dims)
    for col_name, col_dim in target_columns.items():
        print("Processing Attribute: {}".format(col_name))
        ...
        models[col_name] = model
    return models
def createAttributeModel(ModelClass, in_dims, attribute, att_values_length, weights_root, use_gpu=None):
    weights_path = os.path.join(weights_root, attribute+".pth")
    load_weights_path = None
    if os.path.exists(weights_path):
        load_weights_path = weights_path
    print(load_weights_path)
    model = load_model(ModelClass, in_dims, att_values_length, weights_path=load_weights_path, use_gpu=use_gpu)
    save_model(model, weights_path)
    return model

def create_attributes_model_for_test_custom(ModelClass, in_dims, target_columns, weights_root,use_gpu=None):
    models = {}
    for col_name, col_dim in target_columns.items():
        print("Processing Attribute: {}".format(col_name))
        weights_path = os.path.join(weights_root, col_name + ".pth")
