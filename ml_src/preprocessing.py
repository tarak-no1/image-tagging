...
def train_valid_test_split(SOURCE_DATA_DIR, TARGET_DATA_DIR, train_size=0.8, valid_size=0.1, 
                           COMBINE_FOLDERS=None, SELECT_FOLDERS=None, train_override=False):
    """
    Usage:
    ...
    print("Splitting images into train, test and valid folders")
...
def load_label_values(label_values_file):
    ...
    for product_line, product_line_data in label_values['idx_to_names'].items():
        for key, values in product_line_data.items():
            label_values["idx_to_names"][product_line][key] = {int(k): v for k, v in values.items()}
...
def get_label_idx_to_name(label_values, product_line, attribute_name, label_idx):
    return label_values["idx_to_names"][product_line][attribute_name][label_dx]
...
class AttributeDatasetNew(data.Dataset):
    def __init__(self, images_folder, transform=None, target_transform=None, loader=default_loader):
    ...
    def _get_data(self):
        ...
        for file_location in glob(os.path.join(self.images_folder, "*.jpg")):
            filename = file_location.split("/")[-1]
            images.append(self.images_folder+"/"+filename)
        return images
    def __getitem__(self, index):
...
def make_dsets(IMAGES_FOLDER, LABELS_FILE, target_column, batch_size=32, num_workers=4, 
    ...
    return dset_loader
def makeDsets(IMAGES_FOLDER, batch_size=32, num_workers=4, 
               is_train=True, shuffle=True):
    
    # Data Augmentation and Normalization
    data_transforms = get_transforms(is_train)

    dset = AttributeDatasetNew(IMAGES_FOLDER, transform=data_transforms)
