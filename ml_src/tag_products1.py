...
def downloadImages(data, image_folder_path):
    ...
    for source in data:
        ...
        db_to_image_map[image_path] = url
        try:
...
def getAttributesTagsForImages(attribute_model, product_line, attribute, docs, inventory_id=None, company_name='general'):
    image_folder_path = None
    print(inventory_id)
    if inventory_id!=None:
        image_folder_path = inventory_images_path+"/"+str(inventory_id)
...
def tagAttributes(inventory_id):
    ...
    for product_line in product_lines:
        ...
        for attr in attributes:
            ...
            print("product count is "+str(product_count))
            if product_count>0:
                ...
                docs = list(collection.find(query).batch_size(500).limit(3000))
...
def bulkImageAttributeTagging(product_line, image_links, company_name='general', require_attributes=[]):
	...
	splited_docs = divideArray(image_links, 32)
	print("Splitted into :{} arrays".format(len(splited_docs)))
	attribute_image_result = {}
	for attribute, model in attribute_models.items():
		attribute_image_result[attribute] = getAttributesTagsForImages(model, product_line, attribute, splited_docs, inventory_id=None,company_name='general')
	return attribute_image_result

def getAttributesForImages(product_line, image_links, company_name='general', require_attributes=[]):
    collection = client[DB_NAME][product_line]
    attributes = require_attributes
    target_dims = getAttributeDims(product_line, attributes)
    weight_type = getWeightsType(product_line)
    ...
    for attr in attributes:
        ...
        if os.path.exists(weight_path):
            weights_root[attr] = weight_path
    response_obj = {}
    attribute_models = create_all_attribute_models(AttributeFCN, 512,pretrained_conv_model, target_dims, weights_root, batch_size=32, num_workers=4, num_epochs=10, use_gpu=use_gpu)
    ...
    for image_path, idx in image_data.items():
        ...
        response_obj[idx].update(result)
    try:
        shutil.rmtree(image_folder_path, ignore_errors=False, onerror=None)
    except:
        print("Error while deleting folder")
